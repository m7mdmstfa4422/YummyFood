...
$('.navBtn ul li').on('click', function () {
    $('.loding').stop(true, true).css('display', 'flex').hide().fadeIn(0).fadeOut(700, function () {
        $('body').css({
            overflow: 'auto'
        });
    });

    $('.navBtn ul li').animate({
        marginTop: '15px',
        opacity: 0
    }, 100);
    ...
    // تغيير الأيقونة إلى bars
    $('.myBar i').removeClass('fa-xmark').addClass('fa-bars');
...
$('.myBar').on('click', function () {
    ...
    if (isShow) {
        ...
        $('.navContainer').animate({
            ...
            // إظهار العناصر بالتسلسل
            $('.navBtn ul li:nth-child(1)').fadeIn(70, function () {
                $('.navBtn ul li:nth-child(2)').fadeIn(70, function () {
                    $('.navBtn ul li:nth-child(3)').fadeIn(70, function () {
                        $('.navBtn ul li:nth-child(4)').fadeIn(70, function () {
                            $('.navBtn ul li:nth-child(5)').fadeIn(70, function () {
                                $('.navBtn ul li:nth-child(6)').fadeIn(70, function () {
                                    isShow = true;
                                });
                            });
                        });
...
var detalis = document.querySelector('.detalis');



//? جزء اظاهر السيكشن من الناف بار
...
document.getElementById('btnBackToAreas').addEventListener('click', () => {
    document.querySelector('.myContantArea').classList.remove('d-none');
    document.querySelector('.myContantAreaFood').classList.add('d-none');
});

document.getElementById('btnBackToIngred').addEventListener('click', () => {
    document.querySelector('.myContantIngredients').classList.remove('d-none');
    document.querySelector('.myContantIngredFood').classList.add('d-none');
});
document.getElementById('btnBackToCatego').addEventListener('click', () => {
    document.querySelector('.myContantCategor').classList.remove('d-none');
    document.querySelector('.myContantCategorFood').classList.add('d-none');
});
...
function loadHome() {
    ...
    async function getHomeApi() {
        ...
        try {
            const reqHome = await fetch('https://www.themealdb.com/api/json/v1/1/search.php?s=');
            const response = await reqHome.json();
            allHome = response.meals
            // console.log(allHome);
    ...
    function displayHome() {
        ...
        for (let i = 0; i < allHome.length; i++) {
            let meal = allHome[i];
            box += `
        <div class="item  col-md-3 col-sm-12 rounded-3 col-10 ms-5 ms-sm-0" data-id="${meal.idMeal}">
            <div class="ms-3  myContant position-relative">
                <img src="${meal.strMealThumb}" class="w-100 rounded-3" alt="">
                <div class="myTextContant text-black text-center rounded-3 align-content-center position-absolute bottom-0 end-0 start-0">
        ...
        document.getElementById('frontSecContant').innerHTML = box;

        // ✅ ربط الضغط على كل عنصر لإظهار التفاصيل
...
function loadsearch() {
    ...
    async function getSearch(Word) {
        const reqSearch = await fetch(`https://www.themealdb.com/api/json/v1/1/search.php?s=${Word}`);
        const response = await reqSearch.json();
        arr = response.meals;
        displayScarch()
        // console.log(arr);
    }

    async function getSearchL(L) {
        const reqSearch = await fetch(`https://themealdb.com/api/json/v1/1/search.php?f=${L}`);
        const response = await reqSearch.json();
        arr = response.meals;
        displayScarch()
        // console.log(arr);
    ...
    function displayScarch() {
        ...
        `;
            document.querySelector('.myContantsearch').innerHTML = box;
            return;
        }
        ...
        for (let i = 0; i < arr.length; i++) {
            const Scarch = arr[i];
            box += `
            <div class="item col-md-3 col-sm-12 rounded-3 col-10 ms-5 ms-sm-0" data-id="${Scarch.idMeal}">
                <div class="ms-3 myContant position-relative">
                    <img src="${Scarch.strMealThumb}" class="w-100 rounded-3" alt="${Scarch.strMeal}">
                    <div class="myTextContant text-black text-center rounded-3 align-content-center position-absolute bottom-0 end-0 start-0">
                        <h3>${Scarch.strMeal}</h3>
                    </div>
                </div>
            </div>
        `;
        ...
        const items = document.querySelectorAll('.myContantsearch .item');
        items.forEach(item => {
            item.addEventListener('click', () => {
                const id = item.getAttribute('data-id');
                getDetails(id);
    ...
    var inputFindLetter = document.getElementById('inputFindLetter');
    inputFindLetter.addEventListener('keyup', function () {
        var L = inputFindLetter.value.trim();
        getSearchL(L)
    });
    var inputFind = document.getElementById('inputFind');
    inputFind.addEventListener('keyup', function () {
        var Word = inputFind.value.trim();
        getSearch(Word)
    });
...
function loadCategories() {
    ...
    async function getCategories() {
        try {
            const reqCateg = await fetch('https://www.themealdb.com/api/json/v1/1/categories.php');
            const response = await reqCateg.json();
    ...
    function displayCategories() {
        ...
        for (let i = 0; i < arr.length; i++) {
            let categ = arr[i];
            box += `
                <div class="col-md-3 col-sm-12 rounded-3 col-10 ms-5 ms-sm-0">
                    <div class="ms-3 myContant position-relative categoryBox" data-category="${categ.strCategory}">
                        <img src="${categ.strCategoryThumb}" class="w-100 rounded-3" alt="${categ.strCategory}">
                        <div class="myTextContant text-black text-center rounded-3 align-content-center position-absolute bottom-0 end-0 start-0">
                            <h3>${categ.strCategory}</h3>
                            <p class="pt-3">${categ.strCategoryDescription}</p>
                        </div>
                    </div>
                </div>`;
        }
        document.querySelector('.myContantCategor').innerHTML = box;

        // إضافة حدث click على الفئات بعد العرض
        const categoryBoxes = document.querySelectorAll('.myContantCategor .categoryBox');
        categoryBoxes.forEach(box => {
            box.addEventListener('click', function () {
                categoryName = this.dataset.category;
                console.log('Selected category:', categoryName);
                getCategoryMeals();
                // إخفاء قائمة الفئات وإظهار قائمة الوجبات
                document.querySelector('.myContantCategor').classList.add('d-none');
            });
        });
    }
    ...
    async function getCategoryMeals() {
        try {
            const req = await fetch(`https://www.themealdb.com/api/json/v1/1/filter.php?c=${categoryName}`);
    ...
    function displayCategoryMeals() {
        ...
        document.querySelector('.myContantCategorFood').innerHTML = box;
        document.querySelector('.myContantCategorFood').classList.remove('d-none');

        // ربط الضغط على الوجبات لعرض التفاصيل
        const items = document.querySelectorAll('.myContantCategorFood .item');
        items.forEach(item => {
            item.addEventListener('click', () => {
                const id = item.getAttribute('data-id');
                getDetails(id);
...
function loadAreas() {
    // console.log("🌍 Areas section loaded");
    let areaName = '';

    async function getArea() {
        try {
            const reqArea = await fetch('https://www.themealdb.com/api/json/v1/1/list.php?a=list');
            const response = await reqArea.json();
            arr = response.meals;
            console.log(arr);
            displayArea();
        } catch (error) {
    ...
    function displayArea() {
        ...
        for (let i = 0; i < arr.length; i++) {
            ...
            box += `
        <div class="col-md-3 col-sm-12 rounded-3 col-10 ms-5 ms-sm-0">
            <div class="ms-3 MY-BgColor areaBox d-flex flex-column justify-content-center align-items-center rounded-3 py-4" data-area="${area.strArea}">
                <i class="fa-solid fa-house-laptop fa-4x mb-2 text-white"></i>
                <h6 class="text-white keyWord">${area.strArea}</h6>
            </div>
        </div>`;
        }

        document.querySelector('.myContantArea').innerHTML = box;

        const areaBoxes = document.querySelectorAll('.areaBox');
        areaBoxes.forEach(box => {
            box.addEventListener('click', function () {
                areaName = this.dataset.area;
                console.log('Selected area:', areaName);
                getAreaName();
                document.querySelector('.myContantArea').classList.add('d-none');
            });
        });
    }
    ...
    function displayAreaName() {
        ...
        document.querySelector('.myContantAreaFood').innerHTML = box;
        document.querySelector('.myContantAreaFood').classList.remove('d-none');
...
function loadIngredients() {
    ...
    async function getIngredients() {
        try {
            const reqIngr = await fetch('https://www.themealdb.com/api/json/v1/1/list.php?i=list');
            const response = await reqIngr.json();
    ...
    function displayIngredients() {
        ...
        for (let i = 0; i < arr.length; i++) {
            ...
            box += `
                <div class="col-md-3 col-sm-12 rounded-3 col-10 ms-5 ms-sm-0">
                    <div class="ms-3 MY-BgColor d-flex flex-column justify-content-center align-items-center rounded-3 py-4 areaBox" data-ingredient="${ingred.strIngredient}">
                        <i class="fa-solid fa-drumstick-bite fa-4x"></i>
                        <h4 class="py-3 text-white">${ingred.strIngredient}</h4>
                        <p>${ingred.strDescription ? ingred.strDescription.split(' ').slice(0, 20).join(' ') + '...' : ''}</p>
                    </div>
                </div>`;
        }
        document.querySelector('.myContantIngredients').innerHTML = box;

        const ingredBoxes = document.querySelectorAll('.myContantIngredients .areaBox');
        ingredBoxes.forEach(box => {
            box.addEventListener('click', function () {
                ingredName = this.dataset.ingredient;
                console.log('Selected ingredient:', ingredName);
                getIngredName();
                // إخفاء قائمة المكونات الأساسية وإظهار نتائج الأكل
                document.querySelector('.myContantIngredients').classList.add('d-none');
            });
        });
    }
    ...
    async function getIngredName() {
        try {
            const req = await fetch(`https://www.themealdb.com/api/json/v1/1/filter.php?i=${ingredName}`);
            const response = await req.json();
            arr = response.meals;
            displayIngredName();
        } catch (error) {
    ...
    function displayIngredName() {
        ...
        document.querySelector('.myContantIngredFood').innerHTML = box;
        document.querySelector('.myContantIngredFood').classList.remove('d-none');

        // ربط الضغط على الوجبات لعرض التفاصيل
        const items = document.querySelectorAll('.myContantIngredFood .item');
        items.forEach(item => {
            item.addEventListener('click', () => {
                const id = item.getAttribute('data-id');
                getDetails(id);
...
// Map بين كل ID والدالة الخاصة به
const sectionFunctions = {
    frontSec: loadHome,
    searchSec: loadsearch,
    categoriesSec: loadCategories,
    areaSec: loadAreas,
    ingredientsSec: loadIngredients
};
...
const observer = new IntersectionObserver((entries) => {
    entries.forEach(entry => {
        if (entry.isIntersecting) {
            ...
            if (sectionFunctions[sectionId]) {
                sectionFunctions[sectionId](); // استدعاء الدالة المناسبة




                observer.unobserve(entry.target); // لا تكرر التنفيذ عند كل ظهور
            }
        }
    });
}, {
    threshold: 0.5 // يشتغل لما يظهر نصف القسم على الأقل
});



Object.keys(sectionFunctions).forEach(id => {
    ...
    try {
        ...
        for (let i = 1; i <= 20; i++) {
            ...
            if (ingredient && ingredient.trim() !== '') {
                ingredients += `<span class="badge bg-success text-white m-1">${measure} ${ingredient}</span>`;
            }
        }

        const detalis = document.querySelector('.detalis');
        const content = `
            <div class="container-fluid ms-3 px-5 position-fixed overflow-auto top-0 start-50 bottom-0 translate-middle-x z-3 bg-dark text-white">
                <header class="d-flex justify-content-between align-items-baseline py-2">
                    <h1 class="h3 py-2 px-5 rounded-end-5 bg-warning text-black fw-bold">Details</h1>
                    <button class="btn-close btn-close-white" id="btnClose"></button>
                </header>
                <div class="row mb-4" id="infoWrapper">
                    <div class="col-md-4">
                        <img src="${meal.strMealThumb}" class="w-100 rounded-3" alt="${meal.strMeal}">
                        <h3 class="mt-3 ms-2">${meal.strMeal}</h3>
                    </div>
                    <div class="col-md-8 ms-md-0 ms-2">
                        <h4>Instructions</h4>
                        <p>${meal.strInstructions}</p>
                        <p> <strong>Area:</strong> <span class="badge bg-info text-black">${meal.strArea}</span></p>
                        <p> <strong>Category:</strong> <span class="badge bg-info text-black">${meal.strCategory}</span></p>
                        <h5 class="mt-3">Ingredients:</h5>
                        ${ingredients}
                        <h3 class="mt-4">Tags:</h3>
                        ${meal.strTags ? meal.strTags.split(',').map(tag => `<span class="badge bg-warning text-black mb-2 ">${tag}</span>`).join('') : '<span class="text-muted ">No Tags</span>'}
                        <div class="">
                            ${meal.strSource ? `<a class="btn btn-outline-warning me-2" target="_blank" href="${meal.strSource}">Source</a>` : ''}
                            ${meal.strYoutube ? `<a class="btn btn-outline-danger" target="_blank" href="${meal.strYoutube}">YouTube</a>` : ''}
                        </div>
                    </div>
                </div>
            </div>
        ...
        detalis.innerHTML = content;
        detalis.classList.remove('d-none');
        detalis.classList.add('d-block');
        document.body.style.overflow = 'hidden';

        document.getElementById('btnClose').addEventListener('click', () => {
            detalis.classList.add('d-none');
            detalis.classList.remove('d-block');
...
function validateField(id) {
    ...
    switch (id) {
        ...
        case 'email':
            ...
            message = ' example@yyy.zzz';
            break;
        case 'phone':
            isValid = regex.phone.test(value);
            message = 'Enter valid Egy Phone Number';
            break;
        case 'age':
            isValid = regex.age.test(value);
            message = 'Age should be between 18 and 99 years.';
            break;
        case 'password':
            isValid = regex.password.test(value);
            message = 'Enter valid password Minimum eight characters, at least one letter and one number:';
            break;
        case 'rePassword':
    ...
    showError(inputs[id], isValid, message);
    return isValid;
}

function showError(input, valid, msg) {
...
function checkForm() {
    const allValid = Object.keys(inputs).every(id => validateField(id));
    const allTouched = ['name', 'email', 'phone', 'age', 'password', 'rePassword'].every(id => touched.has(id));
...
for (let id in inputs) {
    inputs[id].addEventListener('input', () => {
        touched.add(id);
        validateField(id);
        if (id === 'password' || id === 'rePassword') validateField('rePassword');
        checkForm();
    });
}